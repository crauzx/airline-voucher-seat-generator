...
- **Backend**: Express + TypeScript, layered as `routes -> service -> repository/seat-generation`:
  ...
  - `app.ts` builds the Express app without starting a listener (so tests can `supertest` it directly against an in-memory database); `index.ts` is the real process entrypoint that opens the SQLite file, runs migrations, and starts listening.
...
cd backend && npm test
```
Jest + Supertest: seat-generation validity/uniqueness, the DB unique-constraint duplicate check, service-layer race-safety, and both routes' happy/error paths.

```bash
cd frontend && npm test
```
Vitest + React Testing Library: date/validation utilities, the API client's error parsing, the full check-then-generate form flow (including the duplicate and validation-error paths), and the 404 page.
...
| `RATE_LIMIT_MAX`        | `60`                                           | Requests allowed per client per window on `/api`              |
